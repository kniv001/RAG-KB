...
**外网已经指向 Java 版**（`https://rag-kb-awa.xyz`，经 cloudflared → `127.0.0.1:8080`），
2026-09-16 完成切换。
...
- **本地上下文上限 10240 token**：8GB 显卡上 `qwen3:4b` + `bge-m3` 同时常驻的极限，
  与选哪个模型无关 —— 卡的是 KV cache 的显存。长对话靠的是历史索引与摘要，不是硬撑窗口。
