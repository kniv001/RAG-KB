...
| 入口 | 支持 | 提取方式 |
|---|---|---|
| 上传 | `.txt` `.md` `.markdown` `.pdf` `.docx` `.csv` `.json` `.html` `.htm` | 按后缀分派：PDF→PDFBox、DOCX→POI、CSV→拼成「表头 \| 值」的行、JSON 原样、**HTML→剥标签**、未知后缀按纯文本处理（不报错 —— `.log` `.conf` 这类不该被拦住）。PDFBox 与 POI 都是重量级依赖，**只在真遇到对应格式时才触发类加载** |
| 联网抓取 | 任意网页 | **遍历 DOM 输出 Markdown**（`WebSearchService.toMarkdown`）：h1–h6 → `#`…`######`，列表 → `- `，表格 → `\| `，代码块围栏保留 |
...
> **已知不对称**：上传的 `.html` 走的仍是标签剥离（`<[^>]+>` → 空格），结构同样丢失，
> 和抓取那条路不一致。要补的话，把 `DocumentParser.fromHtml` 也换成遍历 DOM 即可。

**索引管线**：解析 → 切分（下一节）→ **语境行** → 向量化 → 入库，全链路带缓存 ——
解析按**文件内容哈希**缓存、语境行按「文档名 + 块文本」哈希缓存、向量按「文本 + 模型哈希」缓存
