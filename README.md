...
**摘要怎么写——三条都是实测定的**（`tools/summary-*.py`）：
...
- **数字要机械校验**。本机 4b 模型会把个别数字的末位换成冒号（`600 → 60:`、
  `16384 → 1:16384`），**值特异、确定性、提示词治不了**。合并后检查「摘要里的数字能否
  在输入里找到」，找不到就 `warn`（只记日志，不改内容）。
  已知覆盖不到的：**零星丢失**（8 条里偶尔少 1 条，条目数判据看不出来）。
