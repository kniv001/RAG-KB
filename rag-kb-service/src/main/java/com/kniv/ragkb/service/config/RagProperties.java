...
public class RagProperties {
    ...
    public static class Agent {
         ...
         * **推理时不许复述【参考资料】原文**（默认关）。
         *
         * <p>依据（2026-09-22 把思考按句切开量的，工具 `tools/think-structure-read.py`）：
         * 思考里**中位 38% 的字能对上注入材料里的某一句话**（范围 4~45%，由题型决定），
         * 且这部分**集中在前 1/3**（40% / 36% / 24%）—— 也就是"读材料"这一段。
         * 那些字**已经在提示词里**，再写一遍不产生新信息。天花板：全消掉 ⇒ 思考 −38%。
         *
         * <p><b>它与已判死的五次尝试不同类</b>：那五次是<b>形状</b>约束
         * （条目化 / schema / assistant 预填），这条是<b>内容禁止</b>。
         * 但"读一遍"也可能正是模型组织答案的必要过程 ⇒ 速度与质量都要量，
         * 不能只看思考变短。
         */
        private boolean noRestate = false;

        /**
         * **地址计划**（默认关）：给参考资料里每句话一个地址 ⟨块号.句号⟩，
         * 并要求推理时用地址指代、不要抄原句。
         *
         * <p>依据：思考里中位 **38%** 的字是在复述已经在提示词里的资料
         * （`tools/think-structure-read.py`，n=6，范围 4~45%）。
         *
         * <p>⚠️ **与 {@link #noRestate} 不是同一件事** —— 那一条只有禁止，
         * 实测思考反而变长（p=0.007，方向反）。这一条在禁止之前**先给出替代**
        ...
        /**
         * **分层注入**（默认关）：块召回之后，**只在那些块里**按问题挑最相关的若干句，
         * 只把那几句装进提示词 —— 而不是把整块装进去。
         *
         * <p>依据（2026-09-23，`tools/sent-recall.py` 的分层臂 + 三个独立测量）：
         * <ul>
         *   <li>块 top-12 → 句 top-20：**靶子内容保留 30.6%**、精度 **36.8%**（整块注入 21.9%）、
         *       每题 **1416 字**（整块 6339 字）</li>
         *   <li>而**答案只覆盖被引用块内容的 14%（中位）、31%（90 百分位）**（438 个样本）
         *       ⇒ 保留三成，覆盖九成题目的需要</li>
         *   <li>收益两条线：prefill −0.7s；**decode 速率 75.7 → 84.5 tok/s**
         *       （实拟合 `速率 ≈ 94.9 − 0.00338 × 装入token`，r = −0.97）⇒ 合计约 **−2.9s/题**</li>
         * </ul>
         *
         * <p><b>它与前三次失败不同类</b>：那三次是"禁止"或"换个写法"（收益只有便宜的 prefill），
         * 这一条是**真的少给**，而且**decode 速率那条收益线是量出来的**。
         *
         * <p>⚠️ 风险只有一个：**质量**。所以要按答案侧 bench 判（通过率 + 缺陷率），
         * 而不是看思考变短了没有。
         */
        private boolean sentWindow = false;

        /** 分层注入装几句（{@link #sentWindow} 打开时生效）。20 = 实测那档。 */
        private int sentWindowM = 20;
         ...
         * 判得对，剥成光秃秃的分类器反而判不对。
         *
         * <p><b>2026-09-22 起默认 true</b>（一行回退：{@code KB_CONTRACT_IN_CODE=false}）。
         * 开之前按"先说好的标准"跑了两轮真身对照，标准两条都满足。
         *
         * <p><b>速度（硬证据）</b>：TTFT 中位 <b>4.1 → 2.6s</b>（两次独立实验一致，
         * 另一轮 4.0 → 2.6）、总耗时中位 −23~25%、思考总量 −20~25%。
         * TTFT 是**最干净的那个数** —— 它在生成之前，不受答案内容影响；
         * 且 −1.4s 与算过的账对得上（−1.8s 评估 + 0.5s Jev ≈ −1.3s）。
         *
         * <p><b>质量只能说"不低于"</b>：repeat-5 配对 <b>3 胜 2 平 0 负</b>
         * （均值 19.6 → 20.6）；去掉平局做符号检验 <b>p=0.25，未达显著</b>。
         * 连 repeat-3 一起看，**8 次配对一次都没低于过** —— 但这仍不等于"更好"。
         *
         * <p><b>真正显著的那一维是"不再复述类别"</b>：逐字匹配"答案开头是不是分类标签"——
         * 臂 A <b>7/105（五次各漏 2/1/1/1/2，次次都漏）</b>、臂 B <b>0/105</b>，
         * **Fisher 双侧 p ≈ 0.014**。泄漏原文就是 {@code ANSWER_SYSTEM} 自己那行
         * `【乙】知识性问题，且【参考资料】里有内容` 被抄进正文
         * （与早先"给了示例值就会被抄"同一族）。本类的块里那句
         * **「不要再去分析、判断或复述类别」**正好堵住它 ——
         * 基线提示词没这句，因为它本来就要求模型做判断。
         *
         * <p>⚠️ **"质量不低于"与"速度更快"是分开成立的两件事** ——
         * 别把泄漏那一维的显著当成总分更好的证据。
         */
        private boolean contractInCode = true;
