...
public class RagProperties {
    ...
    public static class Agent {
        ...
        /**
         * **分层注入**（**默认开**，2026-09-23 转正）：块召回之后，**只在那些块里**按问题挑
         * 最相关的若干句，只把那几句装进提示词 —— 而不是把整块装进去。
         *
         * <p>依据（`tools/sent-recall.py` 的分层臂 + 三个独立测量）：
         * <ul>
         ...
         * </ul>
         *
         * <p><b>转正复验</b>（2026-09-23，21 题 × 2 次配对，两次同向）：
         * <ul>
         *   <li>装入 token 3781/3748 → **2647/2347（−30%/−37%）**，20/21 题更低（机制性）</li>
         *   <li>生成 token 1788/1611 → **1963/1938（+10%/+20%）**，16/21、15/21 题更高</li>
         *   <li>总耗时 26.0/22.7s → 26.2/23.5s（符号 10:11、9:12）⇒ **没变长**</li>
         *   <li>质量：缺陷率两臂一致（`走对了出口` 各 2/12）</li>
         * </ul>
         *
         * <p>⚠️ <b>机制假说被自己的复验推翻</b>：原本指望"少给资料 ⇒ 少想"，
         * 实测**思考 +16~19%**。真实账是**提示词 token 换成了思考 token** ——
         * 省下的 1134 个 prefill token 只值 0.3s（3800 tok/s），多生成的约 175 个
         * decode token 值 2.2s（80 tok/s），被"上下文变短 ⇒ decode 速率 +5~10 tok/s"抵掉
        ...
        /**
         * **逐块挑句**：每块各出自己最相关的 {@code k} 句（{@code 0} = 关，退回全局 top-{@link #sentWindowM}）。
         *
         * <p>为什么要有它：全局 {@code LIMIT M} 下**没挤进前 M 名的块一句都拿不到**，
         * 而"这块没句子"的退路是**注入整块** —— 实测平均 **2.0 段/次**、29/43 次调用至少一段，
         * 形状与意图相反：**最相关的块被截成几句，最不相关的块反而装全文**。
         * 逐块挑句后每个有句子的块至少出一句，退路只剩"真的切不出句子"。
         *
         * <p>预算上界是 **块数 × k**（块数由检索名额决定），所以 k 是直接用 token 换来的参数。
         */
        private int sentChunkK = 0;

        /**
         * **Jev 选择题**（默认关）：用**一次前向**读出「哪一段最能直接回答这个问题」，
         * 把结果作为**事实**注进那一段的标头。
         *
         * <p>思路（用户提的）：思考是「判断—修改—复述」，而**判断用"读"比用"写"便宜 50 倍**
         * （前向 ~3800 tok/s vs decode 77 tok/s）。读概率本来就能读**选择题**，不只 A/B：
         * 首 token 的分布里本来就有全部编号的概率，一次前向就拿到排序 ——
         * 逐块问是/否要 N 次（~1.8s），这样只要 **~0.15s**。
         *
         * <p>依据：**锚到块号的思考占 37%**，其中"判断/指路"那一档三个口径量出来 **9~25%**
         * （三个口径各有已知偏，且都朝"把引用算成判断"的方向偏 ⇒ 真值大概率在低端）。
         *
         * <p>⚠️ **只标正向**：给不相关的段标「否」会伤多跳题 —— 单块不够 ≠ 这块没用。
         *
         * <p>⚠️ **先例是负的**：`ctx-in-prompt`（把「本段可回答：…」注入）也是"预先告诉模型
         * 每段是什么"，实测思考没变（p=0.585）。差别在于 ctx 行是**建索引时预生成的、泛化的**，
         * 而这个判断是**针对这个问题、由作答模型自己**给出的。
         */
        private boolean jevPick = false;

        /**
         * **按角度拆 + 按角度装配**（默认关）。
         *
         * <p>与既有的"规划成 1~3 个检索查询"的区别是**多了一层结构**：那个的查询之间平权、
         * 召回来拍成一堆；这个产出「角度 + 查询」，于是可以**按筐给名额**。
         *
         * <p>依据（2026-09-24 实测）：拍平之后**谁声音大谁占满名额** —— 问"金价最近怎么样"时
         * 召回 24 段里 7 段是同一件事的新闻、只有 1 段是用户自己的资料。
         *
         * <p>⚠️ 这一版**只做拼装、不让模型写新文档**：中间多一层模型生成的摘要，
         * 引用链就多一跳，而"每句都能指回某块"是这套系统唯一的信任边界。
         */
        private boolean aspects = false;

        /**
         * Jev 选择题的**注入闸门**：最高概率低于这个值就**不注入**。
         *
         * <p>依据（实测，n=6 grounded）：拿臂 B 选中的段去对臂 A（**没有注入**）的引用 ——
         * **相同的 4 次 P 都在 0.65 以上，不同的 2 次恰好是 P 最低的两次（0.55 / 0.54）**。
         * ⇒ P 高 = 模型本来就会找到那一段（注入只是省掉扫描）；
         * P 低 = 它在猜（注进去就是**误导**）。
         *
         * <p>取 0.70 是**留余量**：0.65 那一次虽然对上了，但它贴着分界，样本只有 6 个。
         */
        private double jevPickMinP = 0.70;
        ...
        /**
         * **"资料只答了一部分"那一档的措辞**（`off` / `line` / `branch`，**默认 off**）。
         *
         * <p>为什么要有它（2026-09-25 实测）：`grounded-partial` 那类题（资料只答一部分，
         * 最常见的现实情形）里有 **11%** 的答案出现"**说知识库没有 + 却引用了资料里的值**"。
         * 逐例查下来根因不是"契约缺档"：代码判的是【乙】（`enough=true`），
         * 而 `CONTRACT_YI` **本来就写着**「不要提『知识库中没有』」——
         * 是同一份契约里两句话在"部分可答"时**互相打架**：
         * 「资料里确实没给的具体值，**直说资料没给**」 vs 「**不要**提『知识库中没有』」。
         * 模型两句话都在执行：**用后者的措辞，说了前者的事**。
         *
         * <p>所以这一档给两种写法做 A/B：
         * <ul>
         *   <li>{@code line} —— 在【乙】里**补一句边界**，把两句话的矛盾解开</li>
         *   <li>{@code branch} —— 把"部分"**单列成一档**（另一条路：更显眼，但多一档要守）</li>
         * </ul>
         * 判据是"部分可答题上还会不会声称知识库没有"（机械可判、且实测**逐题稳定**：
         * nl-p1 / nl-p4 两轮真跑都挂）。
         */
        private String partialHint = "off";
