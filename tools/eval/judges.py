...
_MISSING = re.compile(
    ...
    # **「没给/没说/没提」这一族**（2026-09-26 加）：`KB_PARTIAL_HINT=bing` 那臂按提示词
    # 写了「**资料里没给** X」，而判据只认 没有/未/不含 ⇒ **我让自己的措辞被误判成"没声明"**。
    # 同一个坑今天已经踩过一次（改措辞前没让判据认得那种措辞），这次一并补上。
    # 实测：1453 条落盘结果里新命中 5 条、**结论翻转 0 条**（只让读数不再说谎）。
    r"|(资料|知识库|参考资料|文献)(中|里)?(都|均|也)?没(给|说|提|写|列|讲)"
    r"|没(明确)?(给出|说明|提及|写[明清]|列[出举])")
# 「以下是通用知识」的标注
_GENERAL = re.compile(
