...
def cmd_audit():
    ...
    # ② 句子索引与语料的对齐（2026-09-25 加）。
    #
    # 为什么是"查一下"而不是"记一个戳"：句子表原先每行存**全库语料戳**，而那个戳
    # **加一篇文档就变** ⇒ 明明只新了几块，整个索引都报陈旧。现在每行存**块内容锚**，
    # 于是"还对不对得上"是**一句 SQL 的事**，任何时候都能问、也不会因为无关的改动而误报。
    # 放在 audit 里，是因为**没人消费的字段等于不存在**（教训：`stale` 与判据一直有、
    # 就是不报，于是模型拿着过期概览否决了手上的资料）。
    import sent_index
